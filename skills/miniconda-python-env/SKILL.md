...
description: Use when the user wants to RUN Python code, INSTALL a third-party Python package, or execute a .py / .ipynb file on this machine — explicit ("用 Python 处理", "pip install X", "装个 numpy", "跑这个 notebook", "run this script") OR implicit (PDF text extraction, OCR, data conversion, JSON↔CSV/Excel, plotting, scraping, ML training / inference, batch file automation natural in Python). MUST use Miniconda to create an isolated env per task — never the system Python. Classify each task as A (temp / auto-deleted), B (standalone keeper), or C (formal / long-lived project, Python or mixed-language). Temp env root is user-configurable (default D:\Projects\Claude\Temp\<env-name>\). Do NOT use for: code reading / review with no execution ("看下 main.py 第几行"), concept questions ("what is X", "explain Y", "学 X"), technology comparisons ("Python vs Rust", "conda vs pip", "Anaconda vs Miniconda"), Python version / env info lookups ("python 版本怎么看", "which python is running"), projects already using poetry / uv / pipenv / pyenv-win / Anaconda explicitly, user already in their own activated venv, or env cleanup / uninstall operations ("删 conda 环境", "uninstall conda"). If Miniconda is missing, chain into windows-tools-install-manager.
---
